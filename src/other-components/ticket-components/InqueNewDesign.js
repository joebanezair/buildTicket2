...
const InqueNewDesign = () => {
  ...
  const deleteQueConfirm =(index)=>{
    setDelModal(false);
    const referenceQueue = ref(database, 'InqueTickets/'+index);
    remove(referenceQueue)
    .then(() => {
      fetchData();
      alert(`Ticket with key ${index} was successfully deleted`);
    })
    .catch((error) => {
...
// AssignedQueue, comments
  ...
  const [ddlete, dddlete] = useState(false);
        ...
        {filteredQueue.length > 0 ? (
            filteredQueue.map((data, index) => (
              <div id='queueticket' key={index}>
                <div id='quetie'>
                 <div><span>{data.priority} {count++}</span> {data.name.slice(0, 8)}...</div> 
                </div>
                  {/*  */}
                    {editingIndex === index ? (
                      <div id="queFormEdit1"> 
                         <div id="qr4svj"><span>{data.priority} {count -1}</span> {data.name}</div>    
                            <form>
                              <div>
                                <input
                                  type="text"
                              ...
                              <div>
                                <label>Description {data.time} - {data.date}</label> <br/>
                                <textarea
                        ...
                        <div id='btn-que'>
                          <button onClick={() => editQue(index)}>edit</button>
                            ...
                            {ddlete === true ? (<>
                              <div id="delete3434">
                                <button onClick={() => {
                                const datA = data.keyID;
                                deleteQueConfirm(datA)
                                }}>Yes</button>
                                <button  onClick={() => {
                                dddlete(false)
                                }}>Cancel</button>
